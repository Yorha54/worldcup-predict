...
function renderMomentum() {
  ...
  if (liveAnalysis) {
    ...
    selectedMatch.process = liveAnalysis.sections.map((text, index) => [selectedMatch.status === "finished" ? "全场" : `${liveAnalysis.minute || "实时"}'`, ["局面总览", "机会质量", "纪律与定位球", selectedMatch.status === "finished" ? "赛后结论" : "后续预测"][index], text]);
...
function eventToMatch(event) {
  ...
  const base = rich || {
    id: `espn-${event.id}`, home: genericTeam(homeEnglish), away: genericTeam(awayEnglish), ...baseline, momentum: [50,56,48,61,53,58,47,62,55,59,52,57], moments: [["赛前", "关注正式首发"], ["上半场", "观察中场控制"], ["下半场", "替补深度可能关键"]], process: [["0–20'", "试探阶段", "双方首先确认压迫与出球方式。"], ["20–45'", "建立节奏", "中场控制力将决定机会数量。"], ["45–70'", "调整阶段", "教练换人会改变比赛强弱。"], ["70–90'", "决定比赛", "体能、定位球与失误最可能决定结果。"]]
  };
